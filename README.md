...
|||||
|-|-|-|-|
|Windows| .Net Core | > ```dotnet build UserService.sln``` |
|| Docker | > ```docker build -t ChordConnect .``` |
...
|||||
|-|-|-|-|
|Windows| .Net Core | > ```dotnet run --project src\UserService.Api\``` |
|| Docker | > ```docker run -d -e [envar=value] -p 5000:80 ChordConnect```|
